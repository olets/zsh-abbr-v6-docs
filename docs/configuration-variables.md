...
`ABBR_EXPERIMENTAL_COMMAND_POSITION_REGULAR_ABBREVIATIONS` | <Badge type="danger">6.4.0</Badge> | integer | If non-zero, experimental support for command position regular abbreviations is enabled. If 1, a warning is printed on shell startup. | 0 | Strictly speaking, can have false-positive identification of separate commands… but that might not be a problem. When enabled, abbreviations which include the characters `;`, `&`, or `|` will not expand. Read [pull request #197](https://github.com/olets/zsh-abbr/pull/197).
...
`ABBR_SPLIT_FN` | <Badge type="danger">6.4.0</Badge> | function | Customize the function which splits the text to the left of the cursor into words. While you could do anything with this, the intended use is to add new word delimiters. This makes it possible, for instance, to support expanding abbreviations inside quotation marks. The function should only set a single variable, `REPLY`. Its value should be an array based on `${(z)*}`, for example `${(z)*//['"]/ )}` to support expanding inside quotation marks. | `{ REPLY=( ${(z)*} ) }` | Abbreviations cannot contain your custom delimiters. The quotation marks example configuration does not play nicely with `abbr 'a:b'=c`.
